...
fn has_val(data: FuncData) -> FuncResult {
    get_params!(
...
fn remove(data: FuncData) -> FuncResult {
    get_params!(
    ...
    match target {
        Value::Num(index) => {
            if index < &0.0 || index >= &(value.len() as f64) {
                return Ok(create_err("Index out of bounds", data.heap));
            }
...
fn replace(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str,
...
fn substr(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str,
...
fn to_upper(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str
        ;data);
    Ok(Value::Str(value.to_uppercase()))
}
fn to_lower(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str
        ;data);
    Ok(Value::Str(value.to_lowercase()))
}
fn len(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str
        ;data);
...
fn char_at(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str,
...
fn into_err(data: FuncData) -> FuncResult {
    get_params!(
        Value::Str(value) = Type::Str
    ;data);
    return Ok(create_err(value, data.heap));
}
fn split(data: FuncData) -> FuncResult {
    get_params!(
            Value::Str(value) = Type::Str
        ;data);
    match data.args.len() {
        ...
        _ => {
            ...
            if seperator.is_empty() {
                return Ok(create_err("Seperator must not be empty", data.heap));
            }
            let split_str = value
                .split(seperator)
                .map(|v| Value::Str(String::from(v)))
